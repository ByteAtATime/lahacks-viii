...
import times from '../standards.json';
...
export const eventToShortName = (event: Event): string => {
  return `${event.length} ${poolSizeToUnit(event.poolSize)} ${normalizeStroke(event.stroke)}`;
};
...
export const formatStroke = (stroke: Stroke) => {
  switch (stroke) {
    case 'free':
      return 'Freestyle';
    case 'back':
